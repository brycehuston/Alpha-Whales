...
// ── Whale-strategy-local modules ─────────────────────────────────────────────
mod execution;
mod exits;
mod webhook;

// ── Core infrastructure (from alpha-agents-core library) ──────────────────────
use alpha_agents_core::{
    bundle_tracker, config, db, dispatcher, error, pool_cache, state, telegram, tipping, types,
    websocket,
};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let (Some(bot_token), Some(chat_id)) = (
        ...
        tokio::spawn(async move {
            telegram::send_startup_alert(&client_clone, &bot_token, &chat_id, &mode_str).await;
        });
    }
