...
impl BotState {
    ...
    pub async fn retain_shadow_position(
        &self,
        mint: &str,
        opened_at_ms: u64,
    ...
    pub async fn release_shadow_position(&self, mint: &str) -> bool {
    ...
    pub async fn prune_expired_shadow_positions(&self, now_ms: u64) {
        let mut positions = self.shadow_positions.write().await;
    ...
    pub async fn has_shadow_position(&self, mint: &str) -> bool {
        self.shadow_positions.read().await.contains_key(mint)
    }

    pub async fn shadow_position_count(&self) -> usize {
        self.shadow_positions.read().await.len()
    }

    pub async fn has_traded_mint(&self, mint: &str) -> bool {
