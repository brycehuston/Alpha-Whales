...
// ── Bundle primitives ─────────────────────────────────────────────────────────
// Defined here (core infrastructure) so both dispatcher and execution.rs
// share one authoritative definition without circular dependency.

/// Errors that can occur during bundle building and serialization.
/// `alpha-whales` execution.rs embeds these via `#[from]` into JitoExecutionError.
...
/// A signed, serialized bundle ready for submission to the Jito Block Engine.
#[derive(Clone)]
...
/// Serialize a signed `VersionedTransaction` into a Jito proto packet.
pub fn transaction_to_proto_packet(
    transaction: &VersionedTransaction,
) -> Result<ProtoPacket, BundleError> {
    let data = bincode::serialize(transaction)
        .map_err(|e| BundleError::TransactionSerialization(e.to_string()))?;

    if data.len() > PACKET_DATA_SIZE {
        return Err(BundleError::TransactionTooLarge {
            actual: data.len(),
            maximum: PACKET_DATA_SIZE,
        });
    }
...
pub fn build_and_sign_bundle_with_alt(
    ...
    .map_err(|error| BundleError::MessageCompilation(error.to_string()))?;

    let transaction = VersionedTransaction::try_new(VersionedMessage::V0(message), &[payer])
        .map_err(|error| BundleError::TransactionSigning(error.to_string()))?;
